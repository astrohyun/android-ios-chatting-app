...
def chat_lobby(page: ft.Page, jwt_token: str, nickname: str, channel: str = "default", channels_list=None, prev_messages=None): ##3.1.
##3.1.
    ...
    async def close_and_switch(new_channel: str):
        ...
        chat_lobby(page, jwt_token, nickname, channel=new_channel, channels_list=channels_list, prev_messages=new_messages) ##3.1.
    ...
    async def fetch_messages(channel_name):
        try:
            response = requests.get(f"http://127.0.0.1:8003/chat/messages?channel={channel_name}")
    ...
    try:
        response = requests.get("http://127.0.0.1:8003/chat/channels")
        if response.status_code == 200:
...
##JK
    ...
    async def ws_handler(): ##3.1.
        global all_users
        ws_url = f"ws://127.0.0.1:8003/ws/chat?channel={current_channel}&token={jwt_token}"
        try:
    ...
    def open_create_channel_dialog():
        ...
        def create_channel(e):
            ...
            if new_channel:
                ...
                try:
                    response = requests.post("http://127.0.0.1:8003/chat/create_channel", json=data)
                    if response.status_code == 200:
    ...
    def upload_file(file_obj):
        try:
            ...
            with open(file_obj.path, 'rb') as file:
                response = requests.post(       #HTTP request  - GET POST 와 같은 HTTP protocol 요청을 처리해주는 모듈
                    "http://127.0.0.1:8003/chat/upload",  # HTTP header
                ...
                if response.status_code == 200:
                    ...
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    loop.run_until_complete(send_ws_message(file_message))
...
if __name__ == "__main__":
    def dummy_launch(page: ft.Page):
        ...
        chat_lobby(page, jwt_token="your_jwt_token_here", nickname="테스트유저")
